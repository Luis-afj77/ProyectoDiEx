...
data_cruda
sujetos_placebo <- data_cruda %>% filter(trt=="placebo") %>% dplyr::select(y,subject,period)
sujetos_progabide <- data_cruda %>% filter(trt=="progabide") %>% dplyr::select(y,subject,period)
...
# placebo línea base
```{r}
placebo_periodo0 <- sujetos_placebo %>% filter(period=="0")
media <- mean(placebo_periodo0$y)
media
mediana <- median(placebo_periodo0$y)
mediana
moda <- mfv(placebo_periodo0$y)
moda
sd <- sd(placebo_periodo0$y)
sd
var <- var(placebo_periodo0$y)
var
q <- quantile(placebo_periodo0$y)
q
iqr <- IQR(placebo_periodo0$y)
iqr
cv <- sd/media
cv
ran <- range(placebo_periodo0$y)
ran
```
