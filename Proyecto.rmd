...
boxplots_proga <- ggplot(Data_condimentada %>% filter(tratamiento=="progabide"), aes(x= periodo, y= crisis, fill= tratamiento)) +
  ...
  coord_cartesian(ylim=c(0, 30))+ labs(title = "Distribución del número de crisis por cada periodo, grupo progabide", x = "periodo", y = "crisis") + theme_minimal() + theme(plot.title = element_text(size = 16, color = "blue", face = "italic", "bold", family ="serif"), text = element_text(size = 9), legend.title = element_blank(), legend.text = element_blank(), legend.position='none')  + 
  stat_summary(
    fun.y = median,
    geom = 'line',
